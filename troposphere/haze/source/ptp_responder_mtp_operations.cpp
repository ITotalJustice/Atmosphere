...
namespace haze {
    ...
    Result PtpResponder::SendObjectPropList(PtpDataParser &rdp) {
        ...
        R_UNLESS(num_properties > 0, haze::ResultOperationNotSupported());

        for (u32 i = 0; i < num_properties; i++) {
            /* Read the object handle (unused?). */
            u32 object_id;
            R_TRY(dp.Read(std::addressof(object_id)));
        ...
        if (format_code == PtpObjectFormatCode_Association) {
            R_TRY(m_fs.CreateDirectory(newobj->GetName()));
            m_send_object_id = 0;
        } else {
            u32 flags = 0;
            ...
            R_TRY(m_fs.CreateFile(newobj->GetName(), prop_list.size, flags));
