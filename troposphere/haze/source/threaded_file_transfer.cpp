...
struct ThreadData {
    ThreadData(s64 off, s64 size, const ReadCallback& _rfunc, const WriteCallback& _wfunc, u64 buffer_size);
...
ThreadData::ThreadData(s64 off, s64 size, const ReadCallback& _rfunc, const WriteCallback& _wfunc, u64 buffer_size)
: rfunc{_rfunc}
, wfunc{_wfunc}
...
, write_offset{off} {
    ...
    ueventCreate(std::addressof(uevent_done), false);
...
Result TransferInternal(s64 off, s64 size, const ReadCallback& rfunc, const WriteCallback& wfunc, Mode mode, u64 buffer_size = BUFFER_SIZE) {
    const auto transfer_size = size - off;

    if (mode == Mode::SingleThreadedIfSmaller) {
        if ((u64)transfer_size <= buffer_size) {
    ...
    buffer_size = std::min<u64>(transfer_size, buffer_size);
    ...
    if (mode == Mode::SingleThreaded) {
        ...
        while (off < size) {
            u64 bytes_read;
            const auto rsize = std::min<s64>(buf.size(), size - off);
            R_TRY(rfunc(buf.data(), off, rsize, &bytes_read));
            if (!bytes_read) {
                break;
            }

            R_TRY(wfunc(buf.data(), off, bytes_read));
        ...
        ThreadData t_data{off, size, rfunc, wfunc, buffer_size};
        ...
        waitSingle(waiterForUEvent(t_data.GetDoneEvent()), UINT64_MAX);
...
Result Transfer(s64 off, s64 size, const ReadCallback& rfunc, const WriteCallback& wfunc, Mode mode) {
    return TransferInternal(off, size, rfunc, wfunc, mode);
}
